...
function tryConnect() {
  ...
  ledger.probeDevice().then((status) => {
...
async function testProtoTxs(txs, userData, one = false) {
  ...
  for (const tx of txs) {
    let out =
      ...
      tx.dataBuffer +
      '\n Tx amount Precision: ' +
      (tx.amountPrecision ?? 8) +
      '\n Tx amount Precision 2: ' +
      (tx.amount2Precision ?? 0) +
      '\n Tx fee Precision: ' +
      (tx.feePrecision ?? 8) +
      '\n Tx json: \n' +
...
async function testProtoOrder(orders, userData, one = false) {
  ...
  for (const order of orders) {
    let out =
      ...
      (order.amountPrecision ?? 8) +
      '\n Order fee Precision: ' +
      (order.feePrecision ?? 8) +
      '\n Tx json: \n' +
      JSON.stringify(tx.jsonView, undefined, 4);
    autoTestEl.append(out);
    try {
...
function _signTransaction() {
  ...
  statusEl.setAttribute('error', false);
  appData.ledger
    .signTransaction(appData.selectedUser, {
      dataType: 4,
      dataVersion: 3,
